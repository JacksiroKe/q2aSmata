...
class qa_html_theme_base
{
	...
	{
		$this->output('<style>.qa-main-heading{display:none;}</style>');
	...
	public function profile_page($user)
	{
		...
		$wallposts = isset($this->content['wall_list']) ? $this->content['wall_list'] : null;

		$this->cover_page($fullname, $handle, $user['user_avatar'], 
		(($user['logedin_user'] == $userid) ? true : false ), $user['user_points']);
		...
		$this->output('<div class="qa-profile-side">');
		$this->output('<ul class="qa-profile-item">');
		if (isset($user['user_info']))
			foreach ($user['user_info'] as $userinfo ) $this->show_detail( $userinfo, 'li' );
		$this->output('</ul>');
		$this->output('</div>');
		
		$this->output('<div class="qa-profile-wall">');
		$this->q_list_and_form($wallposts);
		$this->output('</div>');
	}
	...
	public function action_buttons($actionbtns)
	{
		...
		foreach ($actionbtns['buttons'] as $button => $btn) {
			$this->output('<input type="submit" value="'.$btn['label'].'" '.$btn['tags'].' class="qa-form-profile-btn qa-form-profile-' . $button . '"/> ');
		}
		$this->output('</span>');
	}
