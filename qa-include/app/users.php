...
if (QA_FINAL_EXTERNAL_USERS) {
	 ...
	 * Pass true if user checked 'Reuser me' (either now or previously, as learned from cookie).
	 * @param $handle
	 * @param $sessioncode
	 * @param $reuser
	 * @return mixed
	 */
	function qa_set_session_cookie($handle, $sessioncode, $reuser)
	{
		if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }

		// if $reuser is true, store in browser for a month, otherwise store only until browser is closed
		setcookie('qa_session', $handle . '/' . $sessioncode . '/' . ($reuser ? 1 : 0), $reuser ? (time() + 2592000) : 0, '/', QA_COOKIE_DOMAIN, (bool)ini_get('session.cookie_secure'), true);
	}
	 ...
	 * $reuser states if 'Reuser me' was checked in the login form.
	 * @param $userid
	 * @param string $handle
	 * @param bool $reuser
	 * @param $source
	 * @return mixed
	 */
	function qa_set_logged_in_user($userid, $handle = '', $reuser = false, $source = null)
	{
		if (qa_to_override(__FUNCTION__)) { $args=func_get_args(); return qa_call_override(__FUNCTION__, $args); }
		...
		if (isset($userid)) {
			...
			// Logging in from a second browser will make the previous browser's 'Reuser me' no longer
			...
			qa_set_session_cookie($handle, $sessioncode, $reuser);
	...
	function qa_get_logged_in_userid()
	{
		...
		if (!$qa_logged_in_userid_checked) { // only check once
			...
			if (!empty($_COOKIE['qa_session'])) {
				@list($handle, $sessioncode, $reuser) = explode('/', $_COOKIE['qa_session']);

				if ($reuser)
					qa_set_session_cookie($handle, $sessioncode, $reuser); // extend 'reuser me' cookies each time
	...
	function qa_get_one_user_html($handle, $microdata = false, $favorited = false)
	{
		...
		$url = qa_path_html($handle);
		$favclass = $favorited ? ' qa-user-favorited' : '';
		$mfAttr = $microdata ? ' itemprop="url"' : '';

		$userName = qa_html(qa_get_user_name($handle));
		$userHandle = $microdata ? '<span itemprop="name">' . $userName . '</span>' : $userName;
	...
	function qa_get_user_avatar_html($flags, $email, $handle, $blobId, $width, $height, $size, $padding = false)
	{
		...
		return sprintf('<a href="%s" class="qa-avatar-link">%s</a>', qa_path_html($handle), $html);
	}
	...
	/**
	 * Return full name for user $userid (if not using single sign-on integration)
	 * @param $userid
	 * @return string
	 */
	function qa_get_user_name($handle)
	{
	...
	/**
	 * Return first name for user $userid (if not using single sign-on integration)
	 * @param $userid
	 * @return string
	 */
	function qa_get_first_name($handle)
	{
		$userid = qa_handle_to_userid($handle);
		$userinfo = qa_db_select_with_pending(qa_db_user_account_selectspec($userid, true));
