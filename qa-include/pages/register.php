...
if (qa_clicked('doregister')) {
	...
	if (qa_user_limits_remaining(QA_LIMIT_REGISTRATIONS)) {
		...
		$infirstname = qa_post_text('firstname');
		$inlastname = qa_post_text('lastname');
		$insex = qa_post_text('sex');
		$incountry = qa_post_text('country');
		$inmobile = qa_post_text('mobile');
		...
		if (!qa_check_form_security_code('register', qa_post_text('code'))) {
			...
			if (empty($errors)) {
				...
				$userid = qa_create_new_user($infirstname, $inlastname, $insex, $incountry, $inmobile, $inemail, $inpassword, $inhandle);
...
$qa_content['form'] = array(
	...
	'fields' => array(
		'firstname' => array(
			'label' => qa_lang_html('users/firstname_label'),
			'tags' => 'name="firstname" id="firstname" dir="auto"',
			'value' => qa_html(@$infirstname),
			'error' => qa_html(@$errors['firstname']),
		),
		
		'lastname' => array(
			'label' => qa_lang_html('users/lastname_label'),
			'tags' => 'name="lastname" id="lastname" dir="auto"',
			'value' => qa_html(@$inlastname),
			'error' => qa_html(@$errors['lastname']),
		),

		'sex' => array(
			'label' => qa_lang_html('users/sex_label'),
			'type' => 'select-radio',
			'options' => qa_string_to_array(qa_lang('users/sex_list')),
			'htmlextra' => '&nbsp;&nbsp;',
			'values' => array("1", "2"),
			'tags' => 'name="sex" id="sex" dir="auto"',
			'value' => qa_html(@$insex),
			'error' => qa_html(@$errors['sex']),
		),

		'country' => array(
			'label' => qa_lang_html('users/country_label'),
			'tags' => 'name="country" id="country" dir="auto"',
			'type' => 'select',
			'options' => qa_string_to_array(qa_lang('users/country_list')),
			'value' => 'Kenya',//qa_html(@$incountry),
			'error' => qa_html(@$errors['country']),
		),

		'mobile' => array(
			'label' => qa_lang_html('users/mobile_label'),
			'tags' => 'name="mobile" id="mobile" dir="auto"',
			'value' => qa_html(@$inmobile),
			'error' => qa_html(@$errors['mobile']),
		),
...
foreach ($userfields as $userfield) {
	...
	if ($userfield['flags'] & QA_FIELD_FLAGS_CHECKBOX) {
		$qa_content['form']['fields'][$userfield['title']]=array(
			'label' => '',
			'checklabel' => qa_html($label),
			'tags' => 'required name="field_'.$userfield['fieldid'].'"',
			'type' => 'checkbox',
			'value' => isset($value) ? qa_html($value) : '',
			'error' => qa_html(@$errors[$userfield['fieldid']]),
		);
	} elseif ($userfield['flags'] & QA_FIELD_FLAGS_SELECT || $userfield['flags'] & QA_FIELD_FLAGS_RADIO) {
		$options = qa_string_to_array($userfield['extra']);
		$qa_content['form']['fields'][$userfield['title']]=array(
			'label' => qa_html($label),
			'tags' => 'required name="field_'.$userfield['fieldid'].'"',
			'type' => $userfield['flags'] & QA_FIELD_FLAGS_SELECT ? 'select' : 'select-radio',
			'options' => $options,
			'htmlextra' => '&nbsp;&nbsp;',
			'value' => isset($value) ? qa_html(@$options[$value]) : '',
			'error' => qa_html(@$errors[$userfield['fieldid']]),
		);
	} else {
		$qa_content['form']['fields'][$userfield['title']]=array(
...
if ($show_terms) {
	$qa_content['form']['fields']['terms'] = array(
		'type' => 'checkbox',
		'label' => '',
		'checklabel' => trim(qa_opt('register_terms')),
