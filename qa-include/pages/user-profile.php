...
$useraccount['user_name'] = qa_html(qa_get_user_name($userhtml, false));
$qa_content['title'] = $useraccount['user_name'] . ' ('.$useraccount['handle'].')';
$qa_content['error'] = @$errors['page'];
$useraccount['logedin_user'] = $loginuserid;
$useraccount['user_points'] = (@$userpoints['points'] == 1)
				? qa_lang_html_sub('main/1_point', '1', '1')
				: qa_lang_html_sub('main/x_points', qa_html(qa_format_number(@$userpoints['points'])));
...
$qa_content['profile_actions'] = array(
	'form_tags' => 'method="post" action="' . qa_self_html() . '"',
	'form_hidden' => array('code' => qa_get_form_security_code('user-edit-' . $handle)),
...
if (!QA_FINAL_EXTERNAL_USERS) {
	$usertime = qa_time_to_string(qa_opt('db_time') - $useraccount['created']);
	$joindate = qa_when_to_html($useraccount['created'], 0);
	$hasavatar = qa_get_user_avatar_html($useraccount['flags'], $useraccount['email'], $useraccount['handle'], $useraccount['avatarblobid'], $useraccount['avatarwidth'], $useraccount['avatarheight'], qa_opt('avatar_profile_size'));
	$asavatar = '<img src="./qa-media/user.png" width="200" height="200"/>';
	
	$useraccount['user_avatar'] = $hasavatar ? $hasavatar  : $asavatar;
	$useraccount['user_info']['country'] = qa_lang_html('users/from_label').' '.$useraccount['country'].' '.
		($useraccount['sex'] == 1 ? ' ('.qa_lang('users/sex_male').')' : ' ('.qa_lang('users/sex_female').')');
	$useraccount['user_info']['since_when'] = qa_lang_html('users/user_for').' '.qa_html($usertime . ' (' . qa_lang_sub('main/since_x', $joindate['data']) . ')');

	$qa_content['profile_page'] = array(
		...
		'fields' => array(
			...
			'duration' => array(
				'type' => 'static',
				'label' => qa_lang_html('users/user_for'),
				'value' => qa_html($usertime . ' (' . qa_lang_sub('main/since_x', $joindate['data']) . ')'),
				'id' => 'duration',
			),

			'level' => array(
				'type' => 'static',
				'label' => qa_lang_html('users/user_type'),
				'tags' => 'name="level"',
	...
	if (empty($qa_content['profile_page']['fields']['avatar']['html']))
		unset($qa_content['profile_page']['fields']['avatar']);


	// Private message link

	if (qa_opt('allow_private_messages') && isset($loginuserid) && $loginuserid != $userid && !($useraccount['flags'] & QA_USER_FLAGS_NO_MESSAGES) && !$userediting) {
		$qa_content['profile_page']['fields']['level']['value'] .= strtr(qa_lang_html('profile/send_private_message'), array(
			'^1' => '<a href="' . qa_path_html('message/' . $handle) . '">',
			'^2' => '</a>',
		));
		$useraccount['private_message'] = strtr(qa_lang_html('profile/send_private_message'), array(
			'^1' => '<a href="' . qa_path_html('message/' . $handle) . '">',
			'^2' => '</a>',
		));
	}
	...
	if ($userediting) {
		if (isset($maxlevelassign)) {
			$qa_content['profile_page']['fields']['level']['type'] = 'select';
			...
			$qa_content['profile_page']['fields']['level']['options'] = $leveloptions;
			...
			if (qa_using_categories()) {
				...
				if (!$catleveladd && !count($userlevels)) {
					$qa_content['profile_page']['fields']['level']['suffix'] = strtr(qa_lang_html('users/category_level_add'), array(
						'^1' => '<a href="' . qa_path_html(qa_request(), array('state' => 'edit', 'catleveladd' => 1)) . '">',
						'^2' => '</a>',
					));
				} else {
					$qa_content['profile_page']['fields']['level']['suffix'] = qa_lang_html('users/level_in_general');
				}
				...
				foreach ($userlevels as $userlevel) {
					if ($userlevel['entitytype'] == QA_ENTITY_CATEGORY) {
						...
						$qa_content['profile_page']['fields']['uc_' . $index . '_level'] = array(
						...
						$qa_content['profile_page']['fields']['uc_' . $index . '_cat'] = array();
						...
						qa_set_up_category_field($qa_content, $qa_content['profile_page']['fields']['uc_' . $index . '_cat'],
							'uc_' . $index . '_cat', $fieldnavcategories, @$userlevel['entityid'], true, true);

						unset($qa_content['profile_page']['fields']['uc_' . $index . '_cat']['note']);
					}
				}
				...
				$qa_content['profile_page']['fields']['level']['tags'] .= ' id="level_select" onchange="qa_update_category_levels();"';
			}
		}
		...
		foreach ($userlevels as $userlevel) {
			if ($userlevel['entitytype'] == QA_ENTITY_CATEGORY && $userlevel['level'] > $useraccount['level']) {
				$qa_content['profile_page']['fields']['level']['value'] .= '<br/>' .
	...
	if (count($showpermits)) {
		$qa_content['profile_page']['fields']['permits'] = array(
			'type' => 'static',
	...
	if ($loginlevel >= QA_USER_LEVEL_ADMIN && !qa_user_permit_error()) {
		$useraccount['user_info']['mobile'] = qa_lang_html('users/mobile'). ' '.$useraccount['mobile'].
			'<br><span class="qa-form-wide-note">'.
			($userediting ? '' : qa_lang_html('users/only_shown_admins')).'</span>'; 
		...
		$useraccount['user_info']['email'] = qa_lang_html('users/email_label'). ' '.
			($userediting ? $htmlemail : ('<a href="mailto:' . $htmlemail . '">' . $htmlemail . '</a>')).
			qa_html(@$errors['email']).'<br><span class="qa-form-wide-note">'.($doconfirms ? (qa_lang_html($isconfirmed ? 'users/email_confirmed' : 'users/email_not_confirmed') . ' ') : '') .
				($userediting ? '' : qa_lang_html('users/only_shown_admins')).'</span>';
	}
	...
	if ($loginlevel >= QA_USER_LEVEL_MODERATOR && !qa_user_permit_error()) {
		$useraccount['user_info']['last_seen'] = qa_lang_html('users/last_login_label'). ' '.
		strtr(qa_lang_html('users/x_ago_from_y'), array(
			...
			'<br><span class="qa-form-wide-note">'.($userediting ? null : qa_lang_html('users/only_shown_moderators')).'</span>';

		if (isset($useraccount['written'])) {
			$useraccount['user_info']['last_write'] = qa_lang_html('users/last_write_label').' '.
				strtr(qa_lang_html('users/x_ago_from_y'), array(
			...
			unset($useraccount['user_info']['last_write']);
		}
	}
	...
	foreach ($userfields as $userfield) {
		...
		$qa_content['profile_page']['fields'][$userfield['title']] = array(
	...
	if ($userediting) {
		if ((qa_opt('avatar_allow_gravatar') && ($useraccount['flags'] & QA_USER_FLAGS_SHOW_GRAVATAR)) ||
			...
			$qa_content['profile_page']['fields']['removeavatar'] = array(
				'type' => 'checkbox',
		...
		$qa_content['profile_actions']['buttons'] = array(
			'save' => array(
		...
		$qa_content['profile_page']['hidden'] = array(
			'dosave' => '1',
		...
		if ($approvebutton) {
			$qa_content['profile_actions']['buttons']['approve'] = array(
				'tags' => 'name="doapprove"',
				'label' => qa_lang_html('users/approve_user_button'),
			);
		}

		$qa_content['profile_actions']['buttons']['edit'] = array(
			'tags' => 'name="doedit"',
		...
		if (isset($maxlevelassign) && $useraccount['level'] < QA_USER_LEVEL_MODERATOR) {
			if ($useraccount['flags'] & QA_USER_FLAGS_USER_BLOCKED) {
				$qa_content['profile_actions']['buttons']['unblock'] = array(
					'tags' => 'name="dounblock"',
					'label' => qa_lang_html('users/unblock_user_button'),
				);

				if (!qa_user_permit_error('permit_hide_show')) {
					$qa_content['profile_actions']['buttons']['hideall'] = array(
				...
				if ($loginlevel >= QA_USER_LEVEL_ADMIN) {
					$qa_content['profile_actions']['buttons']['delete'] = array(
				...
				$qa_content['profile_actions']['buttons']['block'] = array(
					'tags' => 'name="doblock"',
	...
	if (!is_array($qa_content['profile_page']['fields']['removeavatar']))
		unset($qa_content['profile_page']['fields']['removeavatar']);
...
$qa_content['user_activity'] = array(
...
if ($loginlevel >= QA_USER_LEVEL_ADMIN) {
	$qa_content['user_activity']['tags'] = 'method="post" action="' . qa_self_html() . '"';

	$qa_content['user_activity']['buttons'] = array(
		'setbonus' => array(
	...
	$qa_content['user_activity']['hidden'] = array(
		'code' => qa_get_form_security_code('user-activity-' . $handle),
	);

} else {
	unset($qa_content['user_activity']['fields']['bonus']);
}

if (!isset($qa_content['user_activity']['fields']['title']['value']))
	unset($qa_content['user_activity']['fields']['title']);

if (qa_opt('comment_on_qs') || qa_opt('comment_on_as')) { // only show comment count if comments are enabled
	$qa_content['user_activity']['fields']['comments'] = array(
		'type' => 'static',
...
if (qa_opt('voting_on_qs') || qa_opt('voting_on_as')) { // only show vote record if voting is enabled
	...
	$qa_content['user_activity']['fields']['votedon'] = array(
		'type' => 'static',
	...
	$qa_content['user_activity']['fields']['votegave'] = array(
		'type' => 'static',
	...
	$qa_content['user_activity']['fields']['votegot'] = array(
		'type' => 'static',
...
if (@$userpoints['points']) {
	$qa_content['user_activity']['fields']['points']['value'] .=
		qa_lang_html_sub('profile/ranked_x', '<span class="qa-uf-user-rank">' . qa_format_number($userrank) . '</span>');
}

if (@$userpoints['aselects']) {
	$qa_content['user_activity']['fields']['questions']['value'] .= ($userpoints['aselects'] == 1)
...
if (@$userpoints['aselecteds']) {
	$qa_content['user_activity']['fields']['answers']['value'] .= ($userpoints['aselecteds'] == 1)
...
if (!QA_FINAL_EXTERNAL_USERS && qa_opt('allow_user_walls')) {
	...
	if ($wallposterrorhtml) {
		$qa_content['wall_list']['error'] = $wallposterrorhtml; // an error that means we are not allowed to post
	} else {
		$qa_content['wall_list']['form']['fields'] = array(
			'message' => array(
		...
		$qa_content['wall_list']['form']['buttons'] = array(
			'post' => array(
	...
	foreach ($usermessages as $message)
		$qa_content['wall_list']['messages'][] = qa_wall_post_view($message);

	if ($useraccount['wallposts'] > count($usermessages))
		$qa_content['wall_list']['messages'][] = qa_wall_view_more_link($handle, count($usermessages));
}


$qa_content['profile_page'] = $useraccount;
