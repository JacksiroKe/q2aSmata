 ...
 * @return mixed
 */
function qa_db_user_create($firstname, $lastname, $sex, $country, $mobile, $email, $password, $handle, $level, $ip)
{
	...
	if (QA_PASSWORD_HASH) {
		qa_db_query_sub(
			'INSERT INTO ^users (created, createip, firstname, lastname, sex, country, mobile, email, passhash, level, handle, loggedin, loginip) ' .
			'VALUES (NOW(), UNHEX($), $, $, $, $, $, $, $, #, $, NOW(), UNHEX($))',
			$ipHex, $firstname, $lastname, $sex, $country, $mobile, $email, isset($password) ? password_hash($password, PASSWORD_BCRYPT) : null, (int)$level, $handle, $ipHex
		);
	} else {
		$salt = isset($password) ? qa_random_alphanum(16) : null;

		qa_db_query_sub(
			'INSERT INTO ^users (created, createip, firstname, lastname, sex, country, mobile, email, passsalt, passcheck, level, handle, loggedin, loginip) ' .
			'VALUES (NOW(), UNHEX($), $, $, $, $, $, $, $, UNHEX($), #, $, NOW(), UNHEX($))',
			$ipHex, $firstname, $lastname, $sex, $country, $mobile, $email, $salt, isset($password) ? qa_db_calc_passcheck($password, $salt) : null, (int)$level, $handle, $ipHex
		);
	}
