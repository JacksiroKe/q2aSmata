...
class qa_html_theme extends qa_html_theme_base
{
	...
	public function adminpanel($sub_navigation)
	{
		if ($this->template == 'admin') {
			$this->output('<div class="qa-adminpanel" id="asm-adminpanel-mobile">');
			...
			if ( count( $sub_navigation ) ) {
				...
				foreach ( $sub_navigation as $key => $sub_navigation_item ) {
					$this->qa_nav_side_bar_item( $sub_navigation_item );
				}
				$this->output( '</div>', '' );
			}
	...
	public function qa_nav_side_bar_item( $nav_item )
	{
		$class = ( !!@$nav_item['selected'] ) ? ' active' : '';
		$icon = ( !!@$nav_item['icon'] ) ? qa_get_fa_icon( @$nav_item['icon'] ) : '';
	...
	{
		// remove sidebar for user profile and admin pages
